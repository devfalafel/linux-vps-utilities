...
#### Whitelisted IPs (For rcon access)
...
#### Allow TCP on Gameservers from Whitelisted IPs
#for ip in $WHITELISTED_IPS; do
    #iptables -A INPUT -p tcp -m multiport --dports $GAMESERVERPORTS -s $ip -j ACCEPT
#done

#### This will catch all typical DoS attempts and Invalid Packet (weak) DDoS attacks as well, as these are never used for the L4D series.
...
iptables -A INPUT -p udp -m multiport --dports $GAMESERVERPORTS -m length --length 50 -j DROP
